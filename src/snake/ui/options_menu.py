...
class OptionsMenu(GameState):
    ...
    def __init__(self, game):
        super().__init__(game)
        self.options = [
            {"name": "Pantalla Completa", "type": "bool", "key": "FULLSCREEN", 
             "value": config.get("WINDOW", "FULLSCREEN", False), "section": "WINDOW"},
            {"name": "Volumen Música", "type": "range", "key": "MUSIC_VOLUME", 
             "value": config.get("AUDIO", "MUSIC_VOLUME", 70), "min": 0, "max": 100, "section": "AUDIO"},
            {"name": "Volumen Efectos", "type": "range", "key": "SFX_VOLUME", 
             "value": config.get("AUDIO", "SFX_VOLUME", 80), "min": 0, "max": 100, "section": "AUDIO"},
            {"name": "Velocidad Inicial", "type": "range", "key": "INITIAL_SPEED", 
             "value": config.get("GAME", "INITIAL_SPEED", 10), "min": 1, "max": 30, "section": "GAME"},
        ]
        self.selected = 0
        self.font = pygame.font.Font(None, 36)
        self.title_font = pygame.font.Font(None, 48)
        self.editing = False
        log_info("Menú de opciones inicializado", "OPTIONS")
    ...
    def _handle_menu_navigation(self, event):
        ...
        if event.key == pygame.K_UP:
            ...
            log_debug(f"Opción seleccionada: {self.options[self.selected]['name']}", "OPTIONS")
            ...
            self._adjust_setting(-1 if event.key == pygame.K_LEFT else 1)
            ...
            if self.options[self.selected]["type"] == "bool":
                ...
                self.editing = True
                log_debug(f"Editando opción: {self.options[self.selected]['name']}", "OPTIONS")
    ...
    def draw(self):
        ...
        title_rect = title.get_rect(center=(config.get("WINDOW", "WIDTH", 800) // 2, 80))
        self.screen.blit(title, title_rect)
        
        # Instrucciones
        instructions = self.font.render("Usa ↑/↓ para navegar, ←/→ para cambiar valores, ESC para volver", 
                                      True, (150, 150, 150))
        instructions_rect = instructions.get_rect(midbottom=(config.get("WINDOW", "WIDTH", 800) // 2, 
                                                          config.get("WINDOW", "HEIGHT", 600) - 30))
        ...
        for i, option in enumerate(self.options):
            ...
            value_color = (0, 255, 0) if i == self.selected and self.editing else (200, 200, 200)
            ...
            if i == self.selected and self.editing:
                pygame.draw.rect(self.screen, (255, 255, 0), 
                               (490, y_pos - 5, value_render.get_width() + 20, 40), 2)
